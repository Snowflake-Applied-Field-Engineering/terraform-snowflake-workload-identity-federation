...
variable "azure_service_principal_id" {
  description = "Optional Override - Azure AD Service Principal (Application) ID to use for WIF AND to attach to the VM's managed identity (if not provided, uses VM managed identity)"
  type        = string
  default     = null
}
