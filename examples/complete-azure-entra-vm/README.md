...
| <a name="input_azure_service_principal_id"></a> [azure\_service\_principal\_id](#input\_azure\_service\_principal\_id) | Optional Override - Azure AD Service Principal (Application) ID to use for WIF AND to attach to the VM's managed identity (if not provided, uses VM managed identity) | `string` | `null` | no |
...
| <a name="input_name_prefix"></a> [name\_prefix](#input\_name\_prefix) | Prefix to apply to resource names. Note that hyphens will be replaced with underscores for Snowflake role and user names. | `string` | `"snow-tf-wif-test-az"` | no |
| <a name="input_os_image"></a> [os\_image](#input\_os\_image) | OS image to use for VM | <pre>object({<br/>    publisher = string<br/>    offer     = string<br/>    sku       = string<br/>    version   = string<br/>  })</pre> | <pre>{<br/>  "offer": "debian-13",<br/>  "publisher": "debian",<br/>  "sku": "13",<br/>  "version": "latest"<br/>}</pre> | no |
...
| <a name="output_z_test_instructions"></a> [z\_test\_instructions](#output\_z\_test\_instructions) | Instructions for testing the WIF connection |
<!-- END_TF_DOCS -->
